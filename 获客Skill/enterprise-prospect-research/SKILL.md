...
description: 基于公开、可追溯企业信息开展产品导向的潜客研究，使用结构化产品策略、五通道候选发现、多源主体核验、事件去重、反向审查和固定评分，直接生成 Sales Buddy CRM 可导入的 crm-customer-list.v1.json。用户提出“为某产品找潜客”“筛选目标企业”“发现近期采购或扩张信号”“生成获客名单、客户画像或 CRM 一键导入清单”时使用；适合从开放市场发现企业，也适合研究用户提供的候选名单。
---
...
CRM `stage` 固定为 `lead`，`grade` 仅写 A/B/C。评分明细和扣分原因保留在 `lead-list.md`。

## 7. 生成 CRM 原生 JSON

读取 `references/output-contract.md` 和 `references/crm-customer-list.v1.schema.json`，生成 `crm-customer-list.v1.json`。直接输出顶层 `customers[]`；不要生成 `leads[]`、`identity`、`research`、独立 `evidence[]` 或 `proposed_crm_import`。

公开事实进入 `fields`、`orgChain`、`marketNews`、`hiringSignals`、`bidding`、`qualifications`、`businessBrief` 和 `painChain`。每个 `fields.*` 必须使用 `{ v, source, confidence, verifiedAt }`。
...
```text
node scripts/validate-crm-json.mjs <crm-customer-list.v1.json>
node scripts/audit-prospect-quality.mjs <crm-customer-list.v1.json>
```

第一个脚本校验完整固定契约；第二个脚本检查重复企业、证据覆盖、A/B 信号时效、未知项和私有数据边界。失败时修正输出，不得绕过。

从同一研究结果生成 `lead-list.md`，包含策略、工具覆盖、发现通道、评分、事实、需求推测、反向审查、未知项和 `manual_review`。JSON 与 Markdown 的企业名称、等级和数量必须一致。

推荐交付：

```text
crm-customer-list.v1.json  必须，CRM 一键导入
