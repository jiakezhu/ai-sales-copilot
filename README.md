...
- **批量导入客户**：支持 CRM 原生 JSON、CSV、TSV、XLSX、XLS，提供导入预览、逐条校验和同名客户更新/跳过策略。
...
- **周期复盘**：分析页支持本周、上周、本月、上月切换，自动汇总跟进、阶段推进、完成/逾期待办、成交流失、关键进展、风险和下期行动，并可复制规则总结或调用 AI 润色。
...
- **批量导入**：客户页支持 `crm-customer-list.v1.json`、CSV、TSV、XLSX、XLS，导入前预览校验；同名客户可选择跳过或更新。提醒日期会归一化为 `YYYY-MM-DD`，无效日期作为行级错误提示。
- **AI API**：`POST /api/ai/extract` 用于销售信息结构化抽取，`POST /api/ai/polish-review` 用于周期总结表达润色；API Key 不下发浏览器。未配置或润色结果未通过事实校验时，前端继续保留规则总结。接口按用户和 IP 限制每分钟及每日调用量。
